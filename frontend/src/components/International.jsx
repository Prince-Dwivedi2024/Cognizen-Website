...
const MoreInInternationalCard = ({ type, description, title, photo, author, publishDate, onReadMore }) => {
  const limitedDescription = (description) => {
    const words = description.split(' ');
    if (words.length > 25) {
...
const International = () => {
  ...
  useEffect(() => {
      const fetchInternationalArticles = async () => {
          try {
              const response = await fetch('https://cognizen-website.onrender.com/getinternational?type=Article');
  ...
  return (
      <>
          ...
          <div className="min-h-screen bg-[#F0F4F8] p-10 pt-2 flex justify-center shadow-sm">
              <div className="w-4/5">
                  <div className="pb-12">
                      <h2 className="text-3xl font-medium font-serif pt-16 pb-2 text-[#222f3d] underline underline-offset-8">International ARTICLES -</h2>
                  ...
                  <div className="pb-12 border-t border-black pt-10">
                      <h2 className="text-3xl font-medium pb-8 font-serif text-[#222f3d] underline underline-offset-8">MORE IN International -</h2>
                      <div className="grid grid-cols-1 gap-4">
                          {articles.slice(4).map((article, index) => (
                              <MoreInInternationalCard key={index} {...article} onReadMore={() => handleReadMore(article.type)} />
                          ))}
                      </div>
                  </div>
              </div>
