...
const MoreInReviewsCard = ({ type, description, title, photo1, author, publishDate, onReadMore }) => {
  const limitedDescription = (description) => {
    const words = description.split(' ');
    if (words.length > 25) {
...
const Reviews = () => {
  ...
  useEffect(() => {
    const fetchReviewsArticles = async () => {
      try {
        setLoader(true);
        const response = await fetch('https://cognizen-website.onrender.com/getreviews?type=Article');
        ...
        setLoader(false);
        console.error('Error fetching Reviews articles:', error);
      }
    };

    fetchReviewsArticles();
  }, []);
    ...
    <>
      ...
      {loader ? (
        ...
        <div className="min-h-screen bg-[var(--bg-color)] p-10 pt-2 flex justify-center shadow-sm">
          <div className="w-4/5">
            <div className="pb-12">
              <h2 className="text-3xl font-medium font-serif pt-16 pb-2 text-[#222f3d] dark:text-[#A0B4C3] underline underline-offset-8">REVIEWS ARTICLES -</h2>
            ...
            <div className="pb-12 border-t border-black dark:border-[#4A4A4A] pt-10">
              <h2 className="text-3xl font-medium pb-8 font-serif text-[#222f3d] dark:text-[#A0B4C3] underline underline-offset-8">MORE IN REVIEWS -</h2>
              <div className="grid grid-cols-1 gap-4">
                {articles.slice(4).map((article, index) => (
                  <MoreInReviewsCard key={index} {...article} onReadMore={() => handleReadMore(article.id)} />
                ))}
              </div>
            </div>
          </div>
