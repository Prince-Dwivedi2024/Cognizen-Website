...
const Card = ({ type, title, photo1, photo2, author, publishDate, description, onReadMore, large }) => {
  const limitedDescription = (description) => {
    ...
    // Adjust word count based on screen size for responsiveness
    if (isSmallScreen) {
  ...
  return (
    <div
      ...
      style={large ? { width: 'auto', height: 'auto', margin: '0 auto' } : { maxWidth: '95%', margin: '0 auto' }}
    >
      <div>
        <div
...
const MoreInReviewsCard = ({ type, description, title, photo1, author, publishDate, onReadMore }) => {
  ...
  return (
    <div className="flex border-b border-gray-300 dark:border-gray-600 py-1 md:py-2 card" onClick={() => onReadMore(type)}>
