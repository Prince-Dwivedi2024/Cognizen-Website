...
const MoreInReviewsCard = ({ type, description, title, photo, author, publishDate, onReadMore }) => {
  const limitedDescription = (description) => {
    const words = description.split(' ');
    if (words.length > 25) {
                  ...
                  <div className="pb-12 border-t border-black pt-10">
                      ...
                      <div className="grid grid-cols-1 gap-4">
                          {articles.slice(4).map((article, index) => (
                              <MoreInReviewsCard key={index} {...article} onReadMore={() => handleReadMore(article.type)} />
                          ))}
                      </div>
                  </div>
              </div>
